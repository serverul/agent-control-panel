...
## Users
- **Vlad/Benedict** — single user, developer, manages all agents (2+)
- Auth: single-user (JWT), expandable later

## Tech Stack
- **Backend:** FastAPI (Python)
- **Frontend:** React + TailwindCSS + shadcn/ui
- **Database:** PostgreSQL (standalone, separate from all other projects)
- **Real-time:** WebSocket for live agent status + agent chat
...
### 4. Agent Chat and Communication
Agents can talk to each other and to the user.

**Features:**
- Agent-to-agent chat: agents can message each other on specific topics
- User-to-agent chat: send messages to any agent from the dashboard
- Chat monitoring: user can see all agent conversations in real-time
- Context sharing: agents share relevant context in conversations
- Topic-based threads: organize conversations by topic/project

**Chat UI:**
- Slack-like interface with channels per topic
- DM channels between agents
- Global feed showing all agent activity
...
### ChatMessage
- id: str
- channel: str ("general", "project:hartagis", "dm:hermes:openclaw")
- sender: str (agent_id or "user")
- content: str
- timestamp: datetime
- metadata: dict (topic, context, etc.)
...
- POST /api/agents/{id}/heartbeat — agent heartbeat (5 min active, 30 min idle)
...
- GET /api/chat/channels/{channel}/messages — get messages
- POST /api/chat/channels/{channel}/messages — send message
- POST /api/chat/dm/{agent1}/{agent2} — create DM channel
...
- Client: periodic heartbeat (5 min active / 30 min idle) via REST
- Reads: config, memory, cron jobs, stats
- Writes: status updates, log entries, chat messages
...
- Chat bus for agent-to-agent communication
- Topic-based channels for organized discussions
...
- Port: 3100
- Nginx reverse proxy
- PostgreSQL database (standalone, NOT shared with HartaGIS)
...
**Week 1-2:**
- Backend: FastAPI + PostgreSQL (standalone DB)
- Agent model + CRUD
- Cron job model + CRUD (read from Hermes API)
- Basic dashboard (React + Tailwind + shadcn/ui)
...
- Projects view
- Memory viewer
- Basic chat (agent-to-agent + user-to-agent)
